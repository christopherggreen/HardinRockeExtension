...
hrResults05 <- lapply(hr.cases, function(pn,clst,ns,bs) {
...
hrResults01 <- lapply(hr.cases, function(pn,clst,ns,bs) {
    cat("Trial p = ",pn[1]," n = ",pn[2],"\n")
    hrSimParallel(cl=clst, p = pn[1] , n = pn[2], 
...
lapply( hrResults05, function(x) 100*colMeans(x) )

# print column stdevs for each result
lapply( hrResults05, function(x) 100*apply(x,2,sd) )

}

allmeans <- as.data.frame(t(rbind( hr.cases, 
	100*sapply( hrResults05, function(x) colMeans(x) )
)))
row.names(allmeans) <- NULL

allstds <- as.data.frame(t(rbind( hr.cases, 
	100*sapply( hrResults05, function(x) apply(x,2,sd) )
)))
...
allmeans.hrsimold05 <- allmeans
allstds.hrsimold05 <- allstds

save("allmeans.hrsimold05", file="allmeans.hrsimold05.rda")
save("allstds.hrsimold05", file="allstds.hrsimold05.rda")


allmeans <- as.data.frame(t(rbind( hr.cases, 
	100*sapply( hrResults01, function(x) colMeans(x) )
)))
row.names(allmeans) <- NULL

allstds <- as.data.frame(t(rbind( hr.cases, 
	100*sapply( hrResults01, function(x) apply(x,2,sd) )
)))
row.names(allstds) <- NULL
...
allmeans.hrsimold01 <- allmeans
allstds.hrsimold01 <- allstds

save("allmeans.hrsimold01", file="allmeans.hrsimold01.rda")
save("allstds.hrsimold01", file="allstds.hrsimold01.rda")
