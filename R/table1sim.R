table1sim.parallel <- function(cl,p,nn,N,B=10000,alpha=c(0.01,0.025,0.05),
    ...
    blockfcn <- function(m,b) {
      ...
      hr50 <- sapply(alpha, function(a)
        hr05CutoffMvnormal(nn,p,mcd.alpha=X.5X,signif.alpha=a,method="CG")$cutoff.pred)
      hr75 <- sapply(alpha, function(a)
        hr05CutoffMvnormal(nn,p,mcd.alpha=0.75,signif.alpha=a,method="CG")$cutoff.pred)
      hr95 <- sapply(alpha, function(a)
        hr05CutoffMvnormal(nn,p,mcd.alpha=0.95,signif.alpha=a,method="CG")$cutoff.pred)
      ...
      for ( j in 1:b ) {
        ...
        sest <- tryCatch(CovSest(simdata[,,j], method="rocke"), error=function(e) e)
        if ( inherits(sest,"simpleError") ) {
          cat(conditionMessage(sest),"\n",file=lgf, append=TRUE)
   ...
   clusterExport(cl = cl, c("M","B","blockfcn","p","nn","N","mcd.alpha","alpha"),
     envir=sys.frame(pf) )

   blockresults <- parLapply(cl=cl, X=worklist, function(i) blockfcn(m=i[i],b=i[2]))
