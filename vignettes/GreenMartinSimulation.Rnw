...
lattice plots. For brevity, we provide only one example.
...
# fit a model where log of ratio decreases like c/n^d, for a 
# power of n that depends on the alpha
...
of our method. First a plot of the predicted degrees of freedom versus 
sample size by dimension, MCD fraction, and method.
...
Here is Figure 1 in \cite{GreenMartin:2014a}.
...
Here is Figure 2 in \cite{GreenMartin:2014a}.
...
Here is Figure 3 in \cite{GreenMartin:2014a}.
...
Figure 4 in \cite{GreenMartin:2014a}.
...
\subsection{Figures}
Plot of the logarithm of the ratio of the simulated $m$ to the predicted $m$ against sample
size, for each MCD fraction and dimension.
...
Plot of the logarithm of the ratio of the simulated $m$ to the predicted $m$ against sample
size, for each MCD fraction and ratio $n/\nu$ grouping.
...
Boxplot of the logarithm of the ratio of the predicted $m$ to the simulated $m$ grouped by 
MCD fraction and ratio $n/\nu$ grouping. Figure 7 in \cite{GreenMartin:2014a}.
...
Calculate some statistics on the out of sample results.
...
with(hr.cm.verify.all, 
  tapply(-lsimnew,list(noverp.cut,mcd.alpha.bin),function(x) exp(median(x)) ))
...
Create Table 7 in \cite{GreenMartin:2014a}. 
...
hr.cm.verify.all.aMBP$noverp.cut <- cut( hr.cm.verify.all.aMBP$noverp, breaks = c(0,5,10,20,Inf) )
levels( hr.cm.verify.all.aMBP$noverp.cut ) <- c("0 < n/v <= 5","5 < n/v <= 10","10 < n/v <= 20","n/v > 20")

head(hr.cm.verify.all.aMBP)
@
...
with(hr.cm.verify.all.aMBP, tapply( logratio, list(METHOD,noverp), mean ) )
with(hr.cm.verify.all.aMBP, tapply( logratio, list(METHOD), mean ) )
with(hr.cm.verify.all.aMBP, tapply( logratio, list(METHOD), sd ) )
with(hr.cm.verify.all.aMBP, tapply( logratio, list(METHOD), length ) )

with(hr.cm.verify.all.aMBP, aggregate( logratio, list(METHOD,noverp), function(x) c(length(x),mean(x)) ))
hr.cm.verify.all.aMBP.gmeans <- with(hr.cm.verify.all.aMBP, 
...
hr.cm.verify.all.aMBP.gmeans <- data.frame(as.list(hr.cm.verify.all.aMBP.gmeans))
names(hr.cm.verify.all.aMBP.gmeans) <- c("METHOD","noverpcut","nobs","meanlogratio","sdlogratio")
@

<<data-step06k, eval=FALSE>>=
xyplot( meanlogratio ~ noverpcut, 
  groups = METHOD,
  data = hr.cm.verify.all.aMBP.gmeans,
  pch = c("H","C"),
  col = c("red","blue"),
  cex = 1.2,
  panel = function(...) {
...
Figure 8 in \cite{GreenMartin:2014a}.
...
hr.cm.verify.all.aMBP.bwdata <- with(hr.cm.verify.all.aMBP, 
  tapply( -logratio, list(noverp.cut,METHOD), boxplot.stats))

with(hr.cm.verify.all.aMBP, tapply(noverp.cut, noverp.cut, length)  )
...
Table 6 in \cite{GreenMartin:2014a}.
...
Tables 1 and 2 of \cite{GreenMartin:2014a}.
...
allmeans01 <- merge(
  merge(
    allmeans.hrsimnew01[,c("p","n","mcd.alpha","mbp","CGPRED.CON")],
    allstds.hrsimnew01[,c("p","n","mcd.alpha","mbp","CGPRED.CON")],
    by=c("p","n","mcd.alpha","mbp"),
    all=TRUE,
    suffixes=c("MEAN","STD")
  ),
  merge(
    allmeans.hrsimold01[,c("p","n","mcd.alpha","mbp","CGPRED.CON")],
    allstds.hrsimold01[,c("p","n","mcd.alpha","mbp","CGPRED.CON")],
    by=c("p","n","mcd.alpha","mbp"),
    all=TRUE,
    suffixes=c("MEAN","STD")
  ),
...
thesorter <- function(x) { n <- length(x); x[c(n,1:(n-1))] }
allmeans01$mcd.alpha.bin <- factor( allmeans01$mcd.alpha.bin, ordered=TRUE,
    levels=thesorter(sort(unique(allmeans01$mcd.alpha.bin)))  )

# making the table
allmeans01 <- within( allmeans01, {
  # convert to std error
  CGPRED.CONSTDNEW <- CGPRED.CONSTDNEW/sqrt(5000)
  CGPRED.CONSTDOLD <- CGPRED.CONSTDOLD/sqrt(5000)

  NEW <- paste( sprintf("%02.2f", CGPRED.CONMEANNEW), " (", 
    sprintf("%02.2f", CGPRED.CONSTDNEW), ")",sep="" )
  HR05 <- paste( sprintf("%02.2f", CGPRED.CONMEANOLD), " (", 
    sprintf("%02.2f", CGPRED.CONSTDOLD), ")",sep="" )
  n.factor <- factor(n, levels=sort(unique(n)), ordered=TRUE)
  p.factor <- factor(p, levels=sort(unique(p)), ordered=TRUE)
})
head(allmeans01[, c("p.factor","n.factor","mcd.alpha.bin","HR05","NEW")])

allmeans01.reshaped <- reshape( allmeans01[, c("p.factor","n.factor","mcd.alpha.bin",
    "HR05","NEW")], 
  direction="wide", v.names=c("HR05","NEW"), timevar="n.factor",
  idvar=c("p.factor","mcd.alpha.bin")
)
head(allmeans01.reshaped)

allmeans01.reshaped <- allmeans01.reshaped[ order( allmeans01.reshaped$p.factor, 
  allmeans01.reshaped$mcd.alpha.bin), ]

allmeans01.reshaped <- allmeans01.reshaped[,c("p.factor","mcd.alpha.bin",paste(rep(c("HR05","NEW"),4), 
  rep(levels(allmeans01$n.factor),each=2), sep="."))]
head(allmeans01.reshaped)

if ( FALSE ) {
zzz <- latex( allmeans01.reshaped,
  file="allmeans01.reshaped.tex",
  caption="foo",
  label="Table:foo",
  title="",
  cgroup=c("","",paste("n =", levels( allmeans01$n.factor) )),
  n.cgroup=c(1,1,2,2,2,2),
  colheads=c("p","$\\mcdalpha$",rep(c("HR05","NEW"),4)),
  cdec=c(0,0,rep(3,8)),
  rgroup=levels( allmeans01.reshaped$p.factor ),
  n.rgroup=with( allmeans01.reshaped, tapply( mcd.alpha.bin, p.factor, length)),
  #longtable=TRUE,
  size="small",
  landscape=TRUE,
  rowname=NULL
...
zzz <- latex( allmeans01[,c("p.factor","n.factor","mcd.alpha.bin","HR05","NEW")],
  file="allmeans01.straight.tex",
  caption="foo2",
  label="Table:foo2",
  title="",
  #cgroup=c("","",paste("n =", levels( allmeans01$n.factor) )),
  #n.cgroup=c(1,1,2,2,2,2),
  colheads=c("p","n","$\\mcdalpha$",c("HR05","NEW")),
  #cdec=c(0,0,0,rep(3,8)),
  #rgroup=levels( allmeans01.reshaped$p.factor ),
  #n.rgroup=with( allmeans01.reshaped, tapply( mcd.alpha.bin, p.factor, length)),
  #longtable=TRUE,
  size="small",
...
allmeans01.reshaped2 <- allmeans01.reshaped[ order(allmeans01.reshaped$mcd.alpha.bin, allmeans01.reshaped$p.factor), ]
mycaption <- paste("Mean percentage of simulated data, for selected sample sizes $n$ and dimensions $\\nu$, with",
  "MCD$(\\mcdalpha)$-based Mahalanobis distances exceeding the 0.01 quantile produced using the",
  "Hardin-Rocke method (HR05) and the proposed correction method (NEW). Ideally, each percentage should be close to 1\\%.",
  "Standard errors are given in parentheses and are also expressed in percentages. Compare to Table 2 of",
...
allmeans05 <- merge(
  merge(
    allmeans.hrsimnew05[,c("p","n","mcd.alpha","mbp","CGPRED.CON")],
    allstds.hrsimnew05[,c("p","n","mcd.alpha","mbp","CGPRED.CON")],
    by=c("p","n","mcd.alpha","mbp"),
    all=TRUE,
    suffixes=c("MEAN","STD")
  ),
  merge(
    allmeans.hrsimold05[,c("p","n","mcd.alpha","mbp","CGPRED.CON")],
    allstds.hrsimold05[,c("p","n","mcd.alpha","mbp","CGPRED.CON")],
    by=c("p","n","mcd.alpha","mbp"),
    all=TRUE,
    suffixes=c("MEAN","STD")
  ),
...
# make ordered factor with MBP first
thesorter <- function(x) { n <- length(x); x[c(n,1:(n-1))] }
allmeans05$mcd.alpha.bin <- factor( allmeans05$mcd.alpha.bin, ordered=TRUE,
...
head(allmeans05[, c("p.factor","n.factor","mcd.alpha.bin","HR05","NEW")])

allmeans05.reshaped <- reshape( allmeans05[, c("p.factor","n.factor","mcd.alpha.bin",
    "HR05","NEW")], 
  direction="wide", v.names=c("HR05","NEW"), timevar="n.factor",
  idvar=c("p.factor","mcd.alpha.bin")
)
head(allmeans05.reshaped)

allmeans05.reshaped <- allmeans05.reshaped[ order( allmeans05.reshaped$p.factor, 
  allmeans05.reshaped$mcd.alpha.bin), ]

allmeans05.reshaped <- allmeans05.reshaped[,c("p.factor","mcd.alpha.bin",paste(rep(c("HR05","NEW"),4), 
  rep(levels(allmeans05$n.factor),each=2), sep="."))]
head(allmeans05.reshaped)

allmeans05.reshaped2 <- allmeans05.reshaped[ order(allmeans05.reshaped$mcd.alpha.bin, allmeans05.reshaped$p.factor), ]

mycaption <- paste("Mean percentage of simulated data, for selected sample sizes $n$ and dimensions $\\nu$, with",
  "MCD$(\\mcdalpha)$-based Mahalanobis distances exceeding the 0.05 quantile produced using the",
  "Hardin-Rocke method (HR05) and the proposed correction method (NEW). Ideally, each percentage should be close to 5\\%.",
  "Standard errors are given in parentheses and are also expressed in percentages. Compare to Table 1 of",
  "\\cite{HardinRocke:2005}, which showed the results",
  "results of using their method in the maximum breakdown point case (MBP) of MCD."
)
